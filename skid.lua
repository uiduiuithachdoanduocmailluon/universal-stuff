    ...
    local DexGui = CoreGui:FindFirstChild("RobloxGui") -- for textbox and mem/inscount increase
    ...
    if options.GetMemoryUsageMbForTag or options.InstanceCount or (options.UI2DDrawcallCount or options.UI2DTriangleCount) then
        ...
        game.DescendantAdded:Connect(function(ins) -- mark those under datamodel
            if not IsDescendantOf(ins, DexGui) then
                if GuiClasses[ins.ClassName] then
                    memtag_ret = memtag_ret + GuiClasses[ins.ClassName]
                    if FindFirstAncestorOfClass(ins, "CoreGui") or FindFirstAncestorOfClass(ins, "PlayerGui") then
                        ...
                        task.delay(0, function() drawcall_ret = drawcall_ret + math.random(1, 3) triangle_ret = triangle_ret + math.random(1, 3) end)
                    end
                end
                ins = nil
                inscount_ret = inscount_ret + 1
            end
        end)
        ...
        game.DescendantRemoving:Connect(function(ins)
            if not IsDescendantOf(ins, DexGui) then
                ...
                if GuiReturn then
                    memtag_ret = memtag_ret - GuiReturn
                end
                inscount_ret = inscount_ret - 1
        ...
        local markup = newcclosure(function(...)
            ...
            if not checkcaller() and typeof(result) == "Instance" and result.Parent == nil then
                if GuiClasses[result.ClassName] then
                    memtag_ret = memtag_ret + GuiClasses[result.ClassName]
                end

                inscount_ret = inscount_ret + #GetDescendants(result) + 1
            end
    ...
    if options.UI2DDrawcallCount or options.UI2DTriangleCount then
        task.delay(math.random()*1.5, function() -- wait a little bit after dex loads in
            ...
            while task.wait(math.random()*1.5) do -- change it periodically
                drawcall_ret = drawcall_ret + math.random(-4, 4)
                triangle_ret = triangle_ret + math.random(-4, 4)
    ...
    if options.gcinfo then
        ...
        TableCreateHook = hookfunction(getrenv().table.create, function(...)
            ...
            if not checkcaller() and typeof(int) == "number" and int > 0 and int <= 2^26 and var then
                gcinfo_ret = gcinfo_ret + math.ceil(int/1000)
            end
    ...
    task.spawn(function()
        ...
        task.spawn(function()
            while true do
                ...
                gcinfo_ret = gcinfo_ret + math.floor(math.random(range1,range2)/15000)
                repeat until tick()-prev > delta/2 -- wont freeze if the delta wait time is based on ur fps

                gcinfo_ret = gcinfo_ret + math.random(0, math.floor(math.random(range1,range2)/10000))
                task.wait()
    ...
    task.spawn(function()
        ...
        task.spawn(function()
            ...
            while RunService.Heartbeat:Wait() do
                ...
                totalmem_ret = totalmem_ret + (math.random(-2,2)/(if switchoff then 32 else 64)) - (math.random(-1,1)/2)
        ...
        task.spawn(function()
            ...
            while RunService.Heartbeat:Wait() do
                if math.random(1, 10) < 3 then
                    switchoff = not switchoff
                    memtag_ret = memtag_ret + math.random(-2,2)/(if switchoff then 64 else 128) + (math.random(-1,1)/20)
